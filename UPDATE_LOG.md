...
## 3. 시설 생산 모드 DB 동기화 및 오프라인 보상 수정
- **생산 모드 저장 기능**: 시설에서 설정한 생산 모드(레벨)가 DB에 저장되지 않아 재접속 시 초기화되던 문제를 해결했습니다. 이제 `player_facility` 테이블의 `production_mode` 컬럼을 통해 설정이 영구 보존됩니다.
- **오프라인 보상 정확도 향상**: 재접속 시 실제 설정된 생산 모드를 기반으로 보상을 계산하여, 재료 부족 등으로 보상을 받지 못하던 현상을 수정했습니다.
