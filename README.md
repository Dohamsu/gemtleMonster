...
프로젝트의 상세 기획 및 개발 가이드는 `docs/` 디렉토리에서 확인할 수 있습니다.

- **[📖 전체 문서 목차 (Documentation Index)](docs/README.md)**
- **기획서**: [게임 기획 요약](docs/game_design_summary.md), [연금술 시스템](docs/features/alchemy_system.md)
- **가이드**: [콘텐츠 추가 가이드](docs/guides/content_addition.md)
...
- **몬스터 제작**: 성공률 기반 제작 시스템
  - **실험 모드**: 레시피가 없어도 조합 시도 가능
  - **실패 보정**: 실패 시에도 부분 경험치 획득 및 **레시피 힌트** 제공
...
### v1.2.1 (2024-12-09)
- 🐉 **몬스터 상세 정보 시스템** 추가
  - 몬스터 클릭 시 **상세 팝업** (이미지, 상세 스탯, 스킬, 스토리)
  - **잠금 시스템 시각화 강화** (황금빛 테두리 및 글로우 효과)
- 🖼️ **인벤토리 UI 개편**
  - 몬스터 목록 카드 **간소화 및 가독성 개선**
  - 포지션 뱃지 및 잠금 버튼 **배치 최적화**
  - 분해 모드와 상세 보기 모드 분리
