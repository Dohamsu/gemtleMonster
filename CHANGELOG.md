...
### 🧪 테스트 시스템 구축
- **단위/통합 테스트 도입 (`Vitest` + `React Testing Library`)**: 안정적인 기능 추가/수정을 위한 테스트 환경 구성.
- **Login Feature**: 로그인 화면 렌더링, 사용자 입력, Supabase 인증 호출 검증 테스트 구현 및 통과.
- **Alchemy Feature**: 연금술 공방 UI 렌더링, 레시피 선택, 제작 로직 트리거 검증 테스트 구현 및 통과.
- **Shop Feature**: 상점 구매/판매 상호작용 테스트 코드 구현 (현재 환경 설정 이슈 디버깅 중).
